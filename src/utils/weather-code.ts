...
export function getWeatherCodeInfo(weatherCode: number): WeatherCodeInfo {
  // 晴天
  if (weatherCode === 0) {
    return { emoji: '☀️', icon: 'sunny-outline', description: '晴天', isRaining: false };
  }

  // 部分多雲 / 多雲
  if (weatherCode === 1) {
    return { emoji: '🌤️', icon: 'partly-sunny-outline', description: '多雲', isRaining: false };
  }
  if (weatherCode === 2) {
    return { emoji: '⛅', icon: 'partly-sunny-outline', description: '多雲', isRaining: false };
  }
  if (weatherCode === 3) {
    return { emoji: '☁️', icon: 'cloud-outline', description: '陰天', isRaining: false };
  }

  // 霧
  if (weatherCode === 45 || weatherCode === 48) {
    return { emoji: '🌫️', icon: 'cloudy-outline', description: '霧', isRaining: false };
  }

  // 毛毛雨 (51-55)
  if (weatherCode === 51 || weatherCode === 55) {
    return { emoji: '🌦️', icon: 'rainy-outline', description: '毛毛雨', isRaining: true };
  }

  // 小雨 (56-57: 凍毛毛雨)
  if (weatherCode === 56 || weatherCode === 57) {
    return { emoji: '🌦️', icon: 'rainy-outline', description: '凍毛毛雨', isRaining: true };
  }

  // 小雨 (61)
  if (weatherCode === 61) {
    return { emoji: '🌧️', icon: 'rainy-outline', description: '小雨', isRaining: true };
  }

  // 中雨 (63)
  if (weatherCode === 63) {
    return { emoji: '🌧️', icon: 'rainy-outline', description: '中雨', isRaining: true };
  }

  // 大雨 (65)
  if (weatherCode === 65) {
    return { emoji: '⛈️', icon: 'thunderstorm-outline', description: '大雨', isRaining: true };
  }

  // 凍雨 (66)
  if (weatherCode === 66) {
    return { emoji: '🌧️', icon: 'rainy-outline', description: '凍雨', isRaining: true };
  }
  ...
  // 小雪 (71)
  if (weatherCode === 71) {
    return { emoji: '🌨️', icon: 'snow-outline', description: '小雪', isRaining: false };
  }

  // 中雪 (73)
  if (weatherCode === 73) {
    return { emoji: '🌨️', icon: 'snow-outline', description: '中雪', isRaining: false };
  }

  // 大雪 (75)
  if (weatherCode === 75) {
    return { emoji: '❄️', icon: 'snow-outline', description: '大雪', isRaining: false };
  }

  // 雨夾雪 (77)
  if (weatherCode === 77) {
    return { emoji: '🌨️', icon: 'snow-outline', description: '雨夾雪', isRaining: false };
  }

  // 陣雨 (80)
  if (weatherCode === 80) {
    return { emoji: '🌧️', icon: 'rainy-outline', description: '陣雨', isRaining: true };
  }

  // 陣風雨 (81)
  if (weatherCode === 81) {
    return { emoji: '⛈️', icon: 'thunderstorm-outline', description: '陣風雨', isRaining: true };
  }

  // 陣冰雨 (82)
  if (weatherCode === 82) {
    return { emoji: '⛈️', icon: 'thunderstorm-outline', description: '陣冰雨', isRaining: true };
  }

  // 陣小雪 (85)
  if (weatherCode === 85) {
    return { emoji: '🌨️', icon: 'snow-outline', description: '陣小雪', isRaining: false };
  }

  // 陣大雪 (86)
  if (weatherCode === 86) {
    return { emoji: '❄️', icon: 'snow-outline', description: '陣大雪', isRaining: false };
  }

  // 雷暴 (95)
  if (weatherCode === 95) {
    return { emoji: '⛈️', icon: 'thunderstorm-outline', description: '雷暴', isRaining: true };
  }
  ...
  // 強雷暴 (99)
  if (weatherCode === 99) {
    return { emoji: '⛈️', icon: 'thunderstorm-outline', description: '強雷暴', isRaining: true };
  }

  // 其他未定義的代碼
  return { emoji: '❓', icon: 'help-circle-outline', description: '未知', isRaining: false };
}
